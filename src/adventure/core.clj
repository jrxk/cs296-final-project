...
(def the-map
  {:spawn-room {:desc "The walls are made of wood. There is no window. To the west is a door. There is a Japanese sword next to you. Several books and some newspapers are on a desk. Under the desk is a strangely looking stone and a pen."
   ...
   :garden    {:desc "This is a beautiful garden. Lots of flowers. "
              :title "in a garden."
              :dir {:south :room1
                    :east :forest
                    :north :temple}
   ...
   :temple    {:desc "This is the atrium of the temple."
               ...
               :dir {:south :garden
                     :east :cliff1}
               :contents #{}}
   :cliff1    {:desc "Below is an abyss. You can't see the end."
               :title "in a garden."
               :dir {:west :temple
                     :east :death
                     :down :death
...
(defn eat_apple [player]
  (let [inventory (player :inventory)]
    (if (inventory "apple")
      (do (println "You ate the apple! Unfortunately you died because of the poisonous bad apple.")
  ...
  (let [inventory (player :inventory)
    ...
    (if (= nil enemy)
      ...
      (if (and (inventory "sword") (> playerlevel enemylevel))
         (do (println (str "Of course you can fight a " enemy "! You killed the " enemy " and leveled up! You are now level " playerlevel ". After a while, the " enemy "respawned. But don't worry, you know you can defeat it!"))
  ...
  (let [inventory (player :inventory)]
    (if (inventory "box")
      (do (println "It looks like you need to enter a password to open this box. The password contains 4 uppercase letters. You feel like you know the answer or you recently read it somewhere. But what is it?...")
          ...
          (if (= password "PPAP")
            (do (println "Yay! You opened the box and got a hammer! A good tool to break a window!")
